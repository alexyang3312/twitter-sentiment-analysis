...
There are several sections about the sentiment analysis, the first section is data selection, cleaning and pre-processing, the data should be perpared before the data analysis. The second section is feature extraction and vectorization of the text by using an offline model. The thrid section is data analysis, the processed data is divided into train dataset and test dataset, and they are used to train or test 2-3 different models, such as Random Forest, Naive Bayes, and KNN(maybe), after that the models are evaluated and optimized respectively. The final section is data visualization, the analysis results is used to draw two Amercian maps by using Basemap which is a third party package for map visualization in Python, and they could show how people feel about Trump and Biden in different states.
（志鹏，放一张流程图）

## Data selection, cleaning and pre-processing
The raw data were downloaded from Kaggle website, they are two CVS files which are hashtag_donaldtrump.csv (971157 rows of data) and hashtag_joebiden.csv (777078 rows of data). They included loads of tweets about JoeBiden and DonaldTrump. There are a lot of data columns in the CSV files, but only tweet text messages and geographic information are used, therefore other useless columns could be dropped. The data were divided into two training sets and test sets and preprocessed respectively. The text data are read by using Spark API from those CSV files, and then they are splitted to 70% for training and 30% for testing. Moreover, every tweet texts are splitted and filtered, such as remove useless keywords, URLs and special characters. 

The machine learning model provided in Spark MLlib deals with data in vector form, therefore the tweet texts need to convert to vector form. An offline model called Word2Vector is downloaded from the internet, which is other people pre-train the Word2vec model by using the Word2vec function provided by Spark and some words from the text8 file provided by Spark, and it is saved the model to the folder word2vecM_simple, so it could directly call the model when transforming tweets into vectors.
 
Then TextBlob should be used to label every single tweet text in all datasets regardless of whether it's going to be used for training or testing, it is a Python natural language processing package and converts the content of Twitter text into an emotional attribute value, such as positive, neutral and negative. Then there are two important parameters: vectors and the corresponding sentiment value, which means label and features, label and features make up the training data set.

## Data analysis by Random Forest implementation
（需要引入文献）
为什么使用随机森林，情感分析是把推特数据归为若干类：积极，中立，消极，而随机森林正好适用于这种分类算法。

After training dataset are pre-processed and all set, those data are used to train a Random Forest classifier model by using RandomForest.trainClassifier method. There are several parameters of trainClassifier method, such as numClasses, numTrees and maxDepth. There are only three labels which are 0 as positive, 1 as neutral and 2 as negative, and labels should take values {0, 1, ..., numClasses-1}, therefore numClasses = 3. Improving the accuracy of the classifier depends on the features and the tuning of the parameters, and the number and depth of the tree could be adjusted and compared the results to see if it works. A Random Forest model could be obtained after the training process, and the model performance is tested by using the trained model's predict method and test dataset's features, then the test results are predicted labels. The next step is calculating test error rate, the predicted labels are compared with the labels converted from TextBlob and the accuracy is calculated, which means to check whether the classifier returns the same result as what TextBlob returned. The zip method is used to combine two kinds of labels, then using filter method to check how many labels are correctly predict. Finally CrossValidation is used to decide the best parameter to improve the accuracy of the classifier.
