...
The raw data were downloaded from Kaggle website, they are two CVS files which are hashtag_donaldtrump.csv (971157 rows of data) and hashtag_joebiden.csv (777078 rows of data). They included loads of tweets about JoeBiden and DonaldTrump. There are a lot of data columns in the CSV files, but only tweet text messages and geographic information are used. The data were divided into two training sets and test sets and preprocessed respectively.
...
After training dataset are pre-processed and all set, those data are used to train a Random Forest classifier model by using RandomForest.trainClassifier method. There are several parameters of trainClassifier method, such as "numClasses = 3", "numTrees = 6" and "maxDepth = 8". There are only three labels which are 0 as positive, 1 as neutral and 2 as negative, and labels should take values {0, 1, ..., numClasses-1}, therefore numClasses = 3. Improving the accuracy of the classifier depends on the features and the tuning of the parameters, and the number and depth of the tree could be adjusted and compared the results to see if it works. A Random Forest model could be obtained after the training, and the model performance is tested by using the trained model's predict method and test dataset's features, then the labels prediction is completed. The next step is calculating test error rate, the zip method is used to combine predicted labels and features, then using filter method to check how many labels are correctly predict. Finally CrossValidation is used to decide the best parameter to improve the accuracy of the classifier.
