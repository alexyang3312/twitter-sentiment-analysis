...
After the training dataset is preprocessed and all set, those data are used to train a Random Forest classifier model by using RandomForest.trainClassifier method. There are several parameters of the trainClassifier method, such as numClasses, numTrees, and maxDepth. There are only three labels which are 0 as positive, 1 as neutral, and 2 as negative, and labels should take values {0, 1, ..., numClasses-1}, therefore numClasses = 3. Improving the accuracy of the classifier depends on the features and the tuning of the parameters, and the number and depth of the tree could be adjusted and compared to the results to see if it works. A Random Forest model could be obtained after the training process, and the model performance is tested by using the trained model's predict method and test dataset's features, then the test results are predicted labels. The next step is calculating the test accuracy rate, the predicted labels are compared with the labels converted from TextBlob and the accuracy is calculated, which means to check whether the classifier returns the same result as what TextBlob returned. The accuracy_score method is used to check how many labels are correctly predicted. Finally, CrossValidation is used to decide the best parameter to improve the accuracy of the classifier.
